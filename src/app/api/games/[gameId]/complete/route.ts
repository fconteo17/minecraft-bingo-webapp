...
export async function POST(
  ...
  try {
    ...
    if (game.gameType === 'Teams') {
      if (!body.teamName) {
        console.log('[CompleteQuest] Team name required for team game:', { gameType: game.gameType });
        return NextResponse.json(
    ...
    // Handle solo games
      ...
      if (!game.players || !game.players.includes(playerName)) {
        console.log('[CompleteQuest] Invalid player name for game:', { playerName, gamePlayers: game.players });
        return NextResponse.json(
      ...
      if (game.completedQuests[playerName].includes(body.questName)) {
        console.log('[CompleteQuest] Quest already completed:', { questName: body.questName, player: playerName });
        return NextResponse.json(
...
function processQuestCompletion(
  ...
  const questIndex = game.quests.findIndex((q: Quest) => {
    const qName = typeof q.name === 'object' && q.name !== null ? (q.name as { name: string }).name : q.name;
  ...
  const qName = typeof game.quests[questIndex].name === 'object' && game.quests[questIndex].name !== null
