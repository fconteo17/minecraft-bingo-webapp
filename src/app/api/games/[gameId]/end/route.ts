    ...
    if (isRanked) {
      ...
      for (const update of playerUpdates) {
        try {
          ...
          await updatePlayerRank(update.playerId, update.playerName, update.mmr, tier, division);
...
function handleTeamGame(game: Game) {
  ...
  if (team1Quests > team2Quests) {
    ...
    console.log('[EndGame] Tie detected, analyzing completion times:', {
      team1: {
        ...
        completedQuests: team1CompletedQuests.map((q) => ({
          name:
            typeof q.name === 'object' && q.name !== null && 'name' in q.name
    ...
    const team1LastQuest = Math.max(
      ...team1CompletedQuests.map((q) => (q.completedAt ? new Date(q.completedAt).getTime() : 0)),
    );
    const team2LastQuest = Math.max(
      ...team2CompletedQuests.map((q) => (q.completedAt ? new Date(q.completedAt).getTime() : 0)),
    );
...
function handleSoloGame(game: Game) {
  ...
  console.log(
    '[EndGame] Solo tie detected, analyzing completion times for players:',
  ...
  const playerLastQuestTimes = tiedPlayers.map((playerScore) => {
    ...
    const lastQuestTime = Math.max(
      ...playerQuests.map((q) => (q.completedAt ? new Date(q.completedAt).getTime() : 0)),
    );
